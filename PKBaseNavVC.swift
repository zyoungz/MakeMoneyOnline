...
//        self.navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor : UIColor.white]
        let dict:NSDictionary = [NSAttributedString.Key.foregroundColor: UIColor.black,NSAttributedString.Key.font : PingFangMedium(18)]
...
//        self.navigationBar.shadowImage = UIImage()
    ...
    func navigationController(_ navigationController: UINavigationController, didShow viewController: UIViewController, animated: Bool) {
        ...
        if ServerConfigManager.judgeHomeSetAB {
            if viewController is PKAIChatVc {
                PKAssetsAccountSelectView.current?.temporaryShow()
            } else {
                PKAssetsAccountSelectView.current?.temporaryHide()
            }
        }
